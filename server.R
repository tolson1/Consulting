...
function(input, output, session) {
  ...
  uniqueYear <- reactive({
    input$insert
    dbGetQuery(con, "SELECT DISTINCT(strftime('%Y', caseDate)) FROM appeals")[,1]
  })
  ...
  current_frame <- reactive({
    ...
    if(!is.null(input$yearInput)) {
      tempData <- subset(tempData, format(as.Date(tempData$caseDate,'%Y-%m-%d'),'%Y') %in% input$yearInput)
    }
  ...
  observeEvent(input$insert, {
    query <- paste("INSERT INTO appeals VALUES (", nextID(),",'",input$caseDate,"','",input$origin,"','",input$caseName,"','",input$type,"','",input$appealNumber,"','",input$docType,"','",input$enBanc,"','",input$judge1,"','",input$judge2,"','",input$judge3,"','",input$opinion1,"','",input$opinion1Author,"','",input$opinion2,"','",input$opinion2Author,"','",input$opinion3,"','",input$opinion3Author,"','",input$opinion4,"','",input$opinion4Author,"','",input$civilDis1,"','",input$civDis2,"','",input$notes,"','",input$url,"')", sep="")
    cat(query)
    ...
    updateTextInput(session, inputId = "opinion4", value = "")
    updateTextInput(session, inputId = "opinion4Author", value = "")
    updateTextInput(session, inputId = "civilDis1", value = "")
    updateTextInput(session, inputId = "civDis2", value = "")
